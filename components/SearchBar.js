...
export default function SearchBar() {
  ...
  const { getBalance } = useAddress();

  const search = async (val) => {
    if (val?.length === 0) return;

    try {
      await getTransactionReceipt(val);
      Router.push(`/tx/${val}`);
    } catch (error) {}

    try {
      await getBalance(val);
      Router.push(`/address/${val}`);
    } catch (error) {}
  ...
  const checkSearch = async (e, value) => {
